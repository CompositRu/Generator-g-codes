...
def get_ordered_list_of_rows(num_row_y: int, order: str) -> List[int]:
    ...
    Args:
        ...
        order: Тип порядка:
            - 'По очереди' — 0, 1, 2, 3, ...
            - 'Сначала чётные' — 1, 3, 5, ..., 0, 2, 4, ... (пользователь считает с 1, поэтому для него это 2, 4, 6, ..., 1, 3, 5, ...)
            - 'Сначала нечётные' — 0, 2, 4, ..., 1, 3, 5, ... (пользователь считает с 1, поэтому для него это 1, 3, 5, ..., 2, 4, 6, ...)
