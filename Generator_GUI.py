...
def click_setup():
    ...
    add_button = Button(win, text = 'Добавить голову', command = add_widget)
    add_button.grid(columnspan = len(heads['Количество рядов игл на голове'])*2, row=6, sticky=W+E, padx=10, pady=0)
    ...
    def save_data_heads():
        ...
        for head, widget in widgets.items():
            ...
            heads["Количество рядов игл на голове"][head_name] = {}
            heads["Количество рядов игл на голове"][head_name]['X'] = int(widget['X'].get())
            heads["Количество рядов игл на голове"][head_name]['Y'] = int(widget['Y'].get())            
            heads["Количество рядов игл на голове"][head_name]['path'] = widget['path'].get()  

        write_to_json_file('heads.json', heads)

    save_button = Button(win, text = 'Сохранить', bg='ivory4', command = save_data_heads)
    save_button.grid(columnspan = len(heads['Количество рядов игл на голове'])*2, row=7, sticky=W+E, padx=10, pady=10)
...
if __name__ == "__main__":
	...
	window = Tk()  
	window.title("Генератор G кодов для ИП станка v.1.2")
	...
	wd_right = display_right_side(right_desk)
	...
	text_field = Entry(right_desk, width = 8, justify='center')
	...
	text_field = Entry(right_desk, width = 8, justify='center')
	text_field.grid(column=1, row=16, sticky=W+E)
	set_text(text_field, filename)
	wd_left["Имя файла"] = text_field
